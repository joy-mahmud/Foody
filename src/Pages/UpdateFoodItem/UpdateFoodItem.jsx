...
const UpdateFoodItem = () => {
    ...
    const handleUpdateFoodItem = async(e)=>{
        ...
        try {
            const res = await axios.post(`${BASE_URL}/api/update-food/${id}`, data, { headers: { 'Content-Type': 'multipart/form-data' } })
            if (res.status==200) {
    ...
    return (
        <div className='my-5'>
            <form onSubmit={handleUpdateFoodItem} encType='multipart/form-data'
                ...
                <button type='submit' className='cursor-pointer font-semibold w-full bg-slate-950 hover:bg-white hover:text-black border border-slate-950 text-white py-2 px-4 rounded-md transition-all duration-500 focus:ring-2 focus:ring-slate-400 focus:ring-offset-1'>Update Food</button>
            </form>
        </div>
    )
}
